...
namespace ArtGallery.Repositories
{
	public class ArtworkRepository(GalleryDbContext db) : IArtworkRepository
	{
		...
		public async Task<PaginatedResponse<PartialArtworkDTO>> PaginatedQuery(ArtworkQueryParams queryParams, int pageIndex)
		{
			...
			{ query = query.Where(a => EF.Functions.Like(a.Title.ToLower(), $"%{queryParams.Title.ToLower()}%")); }
			if (!string.IsNullOrEmpty(queryParams.Artist))
			{ query = query.Where(a => EF.Functions.Like(a.Artist!.Name.ToLower(), $"%{queryParams.Artist.ToLower()}%")); }
			if (!string.IsNullOrEmpty(queryParams.Period))
			{ query = query.Where(a => EF.Functions.Like(a.Period!.Name.ToLower(), $"%{queryParams.Period.ToLower()}%")); }
		...
		public async Task<List<PartialArtworkDTO>> FindPartial()
		{
			return await _db.Artworks.Join(_db.Artists,
				...
				(artwork, artist) => new PartialArtworkDTO
				(
					artwork.ArtworkId,
